...
public class Disassembler {
	...
	private DisassembleInfoBuilder builder;
	private ChunkReader reader;
	
	private int maxDigitsOfCPIndex;

	private boolean disAttr;
	private boolean disConstantPool;
	...
	public void setDisAttr(boolean disAttr) {
		this.disAttr = disAttr;
	}

	public void setDisConstantPool(boolean disConstantPool) {
		this.disConstantPool = disConstantPool;
	}
	...
	public String disassemble() {
		...
		if (disAttr) disassembleAttributes();
		if (disConstantPool) disassembleConstants();
		disassembleCode();
	...
	private void defClassInstruction() {
		...
		if (classInfo.initializer.isPresent()) {
			disFuncBody(classInfo.initializer.get(), "");
		}
		for (ConstantValue.MethodInfo methodInfo : classInfo.methods) {
			disFuncBody(methodInfo, "method");
		}
		builder.endBlock();
	}
	
	/**
	 * Disassemble the specifed method info.
	 *
	 * @param tag Prefix (method or fun).
	 */
	private void disFuncBody(ConstantValue.MethodInfo methodInfo, String tag) {
		builder.enterBlock(
	...
	private void disUpvalues(ConstantValue.MethodInfo methodInfo) {
