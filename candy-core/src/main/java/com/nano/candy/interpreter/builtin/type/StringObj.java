...
public class StringObj extends CandyObject {
	...
	@NativeMethod(name = "format", varArgsIndex = 0)
	public CandyObject format(CNIEnv env, ArrayObj args) {
		try {
			return valueOf(
				String.format(value, ArrayHelper.toJavaArrayForFormat(env, args)));
